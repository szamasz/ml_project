...
config_test = {
    "apartments": {
        ...
        "exclude_columns": [],
        "include_only_columns": ["city", "squareMeters", "centreDistance", "buildYear", "latitude", "longitude"],
    },
}
...
config_inc_exc = {
    "apartments": {
        ...
        "target": "price",
        "parametrized_columns": [],
        "exclude_columns": ["squareMeters"],
        "include_only_columns": ["city"],
    },
}
config_par_exc = {
    "apartments": {
        "source": "test",
        "target": "price",
        "parametrized_columns": ["city"],
        "exclude_columns": ["squareMeters", "city"],
...
config_par_inc = {
    "apartments": {
        ...
        "parametrized_columns": ["city"],
        "exclude_columns": [],
        "include_only_columns": ["squareMeters", "city"],
    },
}
...
config_2 = {
    "apartments": {
        ...
        "parametrized_columns": [],
        "exclude_columns": [],
        "include_only_columns": ["city", "squareMeters", "centreDistance"],
    },
}
...
config_3 = {
    "apartments": {
        "source": "test",
        "target": "price",
        "parametrized_columns": ["hasStorageRoom", "buildYear", "latitude", "longitude"],
        "exclude_columns": [],
        "include_only_columns": ["city", "squareMeters", "centreDistance"],
    },
}
...
config_4 = {
    "apartments": {
        ...
        "target": "price",
        "parametrized_columns": [],
        "exclude_columns": ["city", "squareMeters", "centreDistance"],
        "include_only_columns": [],
    },
}
...
config_5 = {
    "apartments": {
        "source": "test",
        "target": "price",
        "parametrized_columns": ["buildYear", "latitude", "longitude"],
        "exclude_columns": ["city", "squareMeters", "centreDistance"],
        "include_only_columns": [],
    },
}
...
@pytest.mark.parametrize(
    ...
    [
        (
            config_1,
            all_columns,
            {"remain_columns": remain_1, "parametrized_columns": parametrized_1, "excluded_columns": excluded_1},
        ),
        (
            config_2,
            all_columns,
            {"remain_columns": remain_2, "parametrized_columns": parametrized_2, "excluded_columns": excluded_2},
        ),
        (
            config_3,
            all_columns,
            {"remain_columns": remain_3, "parametrized_columns": parametrized_3, "excluded_columns": excluded_3},
        ),
        (
            config_4,
            all_columns,
            {"remain_columns": remain_4, "parametrized_columns": parametrized_4, "excluded_columns": excluded_4},
        ),
        (
            config_5,
            all_columns,
            {"remain_columns": remain_5, "parametrized_columns": parametrized_5, "excluded_columns": excluded_5},
        ),
    ],
)
...
def test_get_reduced_features(get_dataframes):
    ...
    params = {
        ...
        "n_estimators": 999,
        "max_depth": 13,
        "max_features": 0.5909393194013565,
        "bootstrap": False,
    }
...
def test_save_best_study(get_dataframes, mocker):
    ...
    mocker.patch(
        "mlproject.optunasetup.lib.utils.evaluate_model",
        return_value=("mock_pipeline_object", "mock_signature_object", validation_mape),
    )
    save_best_study(study, experiment_name, X_train, y_train, X_val, y_val, columns, target, mlflow)
    assert mlflow.log_params.call_count == expected_number_of_calls
    mlflow.log_params.assert_has_calls(
        [call({"param1": "value"}), call({"target": target}), call({"hash": sha256(b"abcd").hexdigest()[:1024]})],
    )

    expected_number_of_calls = 2
    assert mlflow.log_metrics.call_count == expected_number_of_calls
    mlflow.log_metrics.assert_has_calls(
        [call({"train_mape": study.best_trial.value * (-1)}), call({"validation_mape": validation_mape})],
    )

    mlflow.sklearn.log_model.assert_called_once_with(
        "mock_pipeline_object",
        artifact_path="ml_project",
        signature="mock_signature_object",
        registered_model_name=experiment_name,
    )

    assert mlflow.log_figure.call_count == expected_number_of_calls
